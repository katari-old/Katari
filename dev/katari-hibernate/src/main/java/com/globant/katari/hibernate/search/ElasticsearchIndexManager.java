...
/** Utility class for index managing.
 *
 * This class is elasticsearch 0.20.x dependent, performs workarounds over
 * fields when creates the index, mapping everything as a multi_field value,
 * because elasticsearch 0.20.x does not allow us to sort over analyzed fields.
 * In the 0.90 version, they correct this. As soon as Hibernate search release
 * the verison 5.0 that will work with Lucene 4, we will be able to remove that
 * 'hack'.
 */
  ...
  public static void createIndex(final Class<?> indexedClass,
    ...
    try {
      ...
      if (indexedClass.isAnnotationPresent(Indexed.class)
          && !indexedClass.isAnnotationPresent(
